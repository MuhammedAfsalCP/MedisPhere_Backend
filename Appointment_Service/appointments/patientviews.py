...
class BookingAppointmentAPIView(APIView):
    ...
    def initiate_payment(self, doctor_email, patient_email, date, slot, amount):
        ...
        try:
            ...
            logger.info(
                f"Payment initiated for {patient_email} - Order ID: {order['id']}"
            )
            return Response(
                {
    ...
    def verify_payment(
        ...
        try:
            ...
            logger.info(
                f"Payment verified successfully for {patient_email} - Payment ID: {razorpay_payment_id}"
            )
            return self.send_booking_request(
    ...
    def send_booking_request(self, doctor_email, patient_email, date, slot, amount):
        ...
        try:
            ...
            def on_response(ch, method, properties, body):
                ...
                if properties.correlation_id == correlation_id:
                    try:
                        ...
                        logger.info(
                            f"Received booking response: {booking_response} - Correlation ID: {correlation_id}"
                        )
                    except json.JSONDecodeError:
                        logger.error(
                            f"Failed to parse JSON response - Correlation ID: {correlation_id}"
                        )
            ...
            logger.info(
                f"Booking request published to slotbooking queue: {request_data} - Correlation ID: {correlation_id}"
            )
            ...
            if booking_response is None:
                logger.error(
                    f"Timeout waiting for booking response - Correlation ID: {correlation_id}"
                )
                return Response(
            ...
            if "error" in booking_response:
                logger.error(
                    f"Booking failed: {booking_response['error']} - Correlation ID: {correlation_id}"
                )
                return Response(
            ...
            logger.info(
                f"Booking successful for {patient_email} - Status: {booking_response.get('status')}"
            )
            return Response(
                {
            ...
            logger.error(
                f"RabbitMQ connection failed: {str(e)} - Correlation ID: {correlation_id}"
            )
            return Response(
...
class DoctorFetching(APIView):
    ...
    def get(self, request, department):
        ...
        try:
            ...
            request_data = json.dumps({"department": department})
            channel.basic_publish(
                exchange="",
...
class Slotfetching(APIView):
    ...
    def get(self, request, id, date):
        ...
        try:
            ...
            # Send request to RabbitMQ queue
            request_data = json.dumps({"id": id, "date": date})
            channel.basic_publish(
                exchange="",
...
class AppointmentBookingViewMore(APIView):
    """API to create an appointment by checking doctor availability via RabbitMQ"""
    ...
    def get(self, request, id):
        ...
        try:
            ...
            channel.basic_publish(
                exchange="",
                routing_key="booking_details_fetching",
                properties=pika.BasicProperties(
                    reply_to=response_queue_name,
                    correlation_id=correlation_id,
                ),
...
class Reschedulig(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            # Send request to RabbitMQ queue
            request_data = json.dumps(
                {"id": id, "date": date, "slot": slot, "patient_email": patient_email}
            )
            channel.basic_publish(
                exchange="",
                routing_key="reschedule",
                properties=pika.BasicProperties(
                    reply_to=response_queue_name,
                    correlation_id=correlation_id,
                ),
        ...
        return Response(
            {
                ...
                "id": doctor_response.get("id"),
            },
            status=status.HTTP_201_CREATED,
        )


class Canceling(APIView):
    """API to create an appointment by checking doctor availability via RabbitMQ"""
    ...
    def post(self, request):
        ...
        try:
            ...
            # Send request to RabbitMQ queue
            request_data = json.dumps({"id": id, "patient_email": patient_email})
            channel.basic_publish(
                exchange="",
                routing_key="cancel",
                properties=pika.BasicProperties(
