...
def on_request(ch, method, properties, body):
    ...
    try:
        ...
        # Prepare response
        response = {
            "Weekly graph": str(weeklygraph)  # Convert to string for JSON serialization
        }
        logger.info(f"Calculated weekly earnings for doctor {doctor_id}: {weeklygraph}")
    ...
    while True:
        try:
            ...
            channel.basic_consume(
                queue="dashboard_weekily_graph", on_message_callback=on_request
            )
